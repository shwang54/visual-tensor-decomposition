...
class DataRunnerMP:
    ...
    def _worker_main(self, task_queue, data_queue, sigma):
        """
        ...
        while True:
            task = task_queue.get()
            sample = self._task_func(task, sigma)
            if sample is None:
                continue
            feed = {}
    ...
    def start_processes(self, sess, n_processes=1, sigma = 1.0):
        ...
        for n in range(n_processes):
            p = multiprocessing.Process(target=self._worker_main, args=(self.task_queue,self.data_queue, sigma))
            p.daemon = True
            p.start()
            self.processes.append(p)
